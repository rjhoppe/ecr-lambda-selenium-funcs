...
def handler(event, context):
    ...
    requests = json.loads(event['body'])['respNum']

    def request__loop(requests):
    ...
    request__loop(requests)

    def thread_loop (requests):
        while requests > 0:
            t1 = threading.Thread(target=request__loop, args=(requests))
            t1.start()
            t2 = threading.Thread(target=request__loop, args=(requests))
            t2.start()
            t3 = threading.Thread(target=request__loop, args=(requests))
            t3.start()
            t4 = threading.Thread(target=request__loop, args=(requests))
            t4.start()

    thread_loop(requests)
